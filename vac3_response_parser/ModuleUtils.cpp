...
int __stdcall GetVACProcIndex(const std::vector<unsigned char> &Module, const std::vector<unsigned char> &VacIn, unsigned long ModuleBase)
{
	...
	std::cout << "Searching for: 0x" << std::hex << *(unsigned long*)VacIn.data() << std::endl;
	...
	for (int i = 0; i < ProcTable.size(); ++i)
	{
		...
		std::cout << "Processing: 0x" << std::hex << ProcTable.at(i)->Checksum1 << std::endl;
...
void CheckPacketDangerous(unsigned char *VacOut, unsigned long *VacOutSize)
{
	...
	for (auto blacklisted : BlacklistProcessName)
	{
		...
		auto it = FindPattern((unsigned long)VacOut, *VacOutSize, (unsigned char*)blacklisted.c_str(), (char*)mask.c_str(), 0);

		if (it)
			throw std::exception(("Packet is dangerous!: " + blacklisted).c_str());
	}
}